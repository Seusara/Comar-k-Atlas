...
-- Correction found during Task 5 execution: crear_factura() is SECURITY INVOKER,
-- so its internal writes to folios_empresa run as the calling authenticated user
-- and are themselves subject to RLS -- "no policies, only the function touches it"
-- was wrong: with zero policies, RLS blocks the function's own writes unconditionally
-- for every caller, not just an edge case. The fix is the same per-empresa policy
-- already used for clientes/productos/facturas, not SECURITY DEFINER (which would
-- bypass RLS for the function's facturas/conceptos writes too, removing the
-- defense-in-depth this design deliberately relies on).
